...
public abstract class StagesAccess {
    ...
    protected final void notifyListeners() {
        ...
        CommonClass.LOGGER.debug("Cleaning up {} handlers", cleanup.size());
        CommonClass.LOGGER.debug("Notifying {} handlers", frozen.size());
        cleanup.forEach(Runnable::run);
        cleanup = frozen.stream().map(listener -> {
            try {
                return listener.onUpdate(this);
            } catch (Exception e) {
                CommonClass.LOGGER.error("Update Handler encountered exception: {}", e.getMessage());
                return null;
            }
